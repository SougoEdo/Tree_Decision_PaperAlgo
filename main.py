"""A small, readable SPO portfolio tree using only NumPy and SciPy.

Install:  python -m pip install numpy scipy
Demo:     python spo_portfolio_tree.py
...
Arrays supplied by the caller (all rows must be aligned):
    ...
    U: (observations, assets), actual or scenario PRE-TRADE weights.
...
Weights are signed exposures as fractions of portfolio equity, NOT orders.
The default permits long/short/flat portfolios with sum(abs(weights)) <= 1.
There is no sum(weights) constraint unless net_exposure is supplied.
Residual cash/collateral and financing earn zero in this simplified objective.
...
class PortfolioConfig:
    ...
    net_exposure: float | None = None  # optionally require sum(weights) == value
    fee_rate: float | tuple[float, ...] = 0.001  # per traded notional, per asset
    l2_penalty: float = 0.01       # concentration/exposure penalty, not a fee
...
@dataclass(frozen=True)
class TreeConfig:
    max_depth: int = 2            # root depth is 0; zero means one leaf
...
class PortfolioOptimizer:
    def __init__(self, n_assets: int, config: PortfolioConfig):
        ...
        if (config.min_weight >= config.max_weight or config.gross_limit < 0
                or config.l2_penalty < 0 or config.solver_tolerance <= 0
                or config.feasibility_tolerance <= 0
                or config.solver_max_iterations < 1):
            raise ValueError("Invalid bounds, penalties, or solver settings.")
...
def demo():
    ...
    # Synthetic training states, NOT holdings generated by this strategy.
    U = rng.uniform(-0.30, 0.30, size=R.shape)

    portfolio = PortfolioOptimizer(n_assets=R.shape[1], config=PortfolioConfig(
        min_weight=-1.0, max_weight=1.0, gross_limit=1.0,
        net_exposure=None, fee_rate=0.001, l2_penalty=0.01,
    ))
    ...
    current_weights = np.array([[0.15, -0.10]])
