...
from planted_threshold_test import (DAYS_PER_YEAR, FEATURES, Case, annual, expected_return_curve,
...
def plot():
    ...
    figures = {"planted_world_one_dataset": figure_world(single, plt),
    ...
    if os.path.exists(RESULTS):
        ...
        figures.update({
            ...
            "maps_rate_vs_edge": figure_maps(results, plt, "edge", EDGES, edge_labels,
                                         "drift / volatility per day (drift per year)"),
            "maps_rate_vs_speed": figure_maps(results, plt, "k", K_VALUES, k_labels,
    ...
    if os.path.exists(SETTINGS_RESULTS):
        ...
        figures.update({"risk_fee_depth_positions": figure_settings_positions(settings, plt, list(SETTINGS)),
                        "risk_fee_depth_maps": figure_settings_maps(settings, plt, list(SETTINGS))})
    if os.path.exists(ENHANCEMENTS_RESULTS):
        ...
        figures.update({
            ...
            "split_options_thresholds": figure_thresholds_by_setting(enhancements, plt, names),
            "split_options_maps": figure_settings_maps(enhancements, plt, names,
