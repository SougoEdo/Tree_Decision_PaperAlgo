...
\keybox{This document describes the training of \code{SPOPortfolioTree} (\code{spo_tree.py}) as it is today, in the order the code runs it: the inputs, the decision problem the tree feeds, the loss, the replay, how a leaf score is fitted, how candidate thresholds are chosen (plain, quantile, refined, soft), how a split is accepted, how the tree is pruned and refitted, and how it is used. Every option added since the first version of the code is off by default, so with the defaults the training is the original one, plus one convention (Section~\ref{sec:changes}). The experiments that test it are in \code{experiments_planted_threshold.tex}.}
